...
class VAE_30m(nn.Module):
    def __init__(self, input_shape=1550, forecast_shape=240, latent_dim=10):
        ...
        self.out_scale1 = 12  # controls model complexity
        ...
        # Decoder (adjusted to forecast 240 features)
        self.decoder = nn.Sequential(
            ...
            nn.Linear(self.out_scale1 * 10, self.forecast_shape),
            nn.BatchNorm1d(self.forecast_shape),
            nn.Tanh()
        )
...
'''class VAE_30m(nn.Module):
    def __init__(self, input_shape=1550, latent_dim=10):
        super(VAE_30m, self).__init__()
...
'''class VAE_30m(nn.Module):
    def __init__(self, input_shape=1550, forecast_shape=240, latent_dim=10):
        ...
        # Decoder (adjusted to forecast 240)
        self.decoder = nn.Sequential(
            nn.Linear(self.latent_dim, self.out_scale1 * 5),
            nn.BatchNorm1d(self.out_scale1 * 5),
            nn.LeakyReLU(),
